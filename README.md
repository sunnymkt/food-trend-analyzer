...
- `data/news_filters.json` — 뉴스 수집 시 걸러낼 블랙리스트 단어 목록 (수동 관리, 자유롭게
  추가/삭제 가능). 제목+요약에 이 단어가 있으면 그 기사는 제외됩니다
...
- **뉴스 필터링은 동음이의어까지는 걸러내지 못합니다.** `data/news_filters.json`
  블랙리스트와 "다른 단어에 우연히 포함된 경우"(예: '마라'↔'막스마라') 방지 로직으로
  카테고리가 명백히 다른 기사는 걸러내지만, 키워드 자체가 여러 뜻을 가진 경우(예: '쑥'은
  나물 이름이자 "쑥 오르다"처럼 "급격히"를 뜻하는 부사이기도 함)는 문맥을 이해하지
  못해 걸러내지 못할 수 있습니다. "식품 관련 단어가 반드시 있어야 한다"는 식의 추가
  필터는 시도해봤으나, 진짜 관련 기사까지 대거 걸러내는 부작용이 더 커서 적용하지
  않았습니다.
